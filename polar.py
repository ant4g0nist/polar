...
class AARCH64(Architecture):
	...
	def get_disas(self, frame, process, pc=None, size=20):
		if not pc:
			pc 		= get_register("pc", frame)

		error 	= lldb.SBError()
		buffer 	= process.ReadMemory(pc, size, error)
...
class InstructionManualCommand(LLDBCommand):
	...
	def description(self):
		return "Explain the disassembly at current frame powered by ChatGPT"
